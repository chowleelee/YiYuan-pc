...
{	
    ...
    public function doSignIn(){
        ...
        // find查询一条记录
        $info = db('user')->field("user_id, user_nickname, status")->where("user_phone='$user_phone' and user_pwd='$user_pwd'")->find();
        ...
        //判断申请行家状态
        if($status_tutor && $status_topic && $status_tutor == 1 && $status_topic == 1){
            Session::set('verify', 'true');
        }else if($status_topic && $status_topic == 0){
            Session::set('topic', 'true');
        }else if($status_tutor && $status_tutor == 0){
    ...
    public function doSignUp(){
        ...
        $data['user_img'] = 'default_avatar.jpg';

        model('User')->save($data);

        $user_id = db('user')->getLastInsID();

        Session::set('info', ['user_id'=>$user_id, 'user_nickname'=>$user_nickname,'status'=>1]);
    }
    ...
    public function topicList(){
        ...
        //查询分类
        $cate_list = model('Category')->getAll();
    ...
    function editAvatar(){
        $user_id = Session::get('info.user_id');
        $name = saveAndgetImgSrc(ROOT_PATH."public/static/index/img/",'user_img');
        db('user')->where("user_id='$user_id'")->setField('user_img', $name);
    }
